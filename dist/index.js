...
//#endregion
//#region node_modules/iconv-lite/backends/node.js
var require_node = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = {
		...
		bytesToResult(bytes, finalLen) {
			if (finalLen === 0) return Buffer.alloc(0);
		...
		rawCharsToResult(rawChars, finalLen) {
			...
			return Buffer.from(rawChars.buffer).slice(rawChars.byteOffset, rawChars.byteOffset + finalLen * Uint16Array.BYTES_PER_ELEMENT).toString("ucs2");
		}
	};
}));
//#endregion
...
var PrintCPCL = class PrintCPCL {
	...
	image(dataArray, { x = 0, meta = {} } = {}) {
		...
		this.data.push(...import_index_node.default.encode(`CG ${meta.byteWidth} ${meta.height} ${x} ${this.currentY} `, "gb18030"), ...dataArray, ...import_index_node.default.encode("\r\n", "gb18030"));
	...
	_pushData(...value) {
		value.forEach((i) => {
			this.data.push(...import_index_node.default.encode(`${i}\r\n`, "gb18030"));
		});
	}
};
...
var PrintPOS = class PrintPOS {
	...
	text_big(value) {
		...
		this.data.push(...import_index_node.default.encode(value, "gb18030"));
	...
	qrcode(value) {
		const bytes = import_index_node.default.encode(value, "gb18030");
		const qrSize = [
			29,
			40,
			107,
		...
		this.data.push(...qrSize, ...qrErr, ...qrData, ...bytes, ...qrRun);
...
var PrintPic = class {
	...
	imgToRaster(rgba, w, h) {
		...
		for (let i = 0; i < buf.length; i += 4) if (buf[i + 3] > 0) hist[buf[i]]++;
		let sum = 0;
		...
		for (let i = 0; i < buf.length; i += 4) if (Math.round(buf[i] * .299 + buf[i + 1] * .587 + buf[i + 2] * .114) < threshold) grayArray.push(1);
		else grayArray.push(0);
